...
def _mcp_stdio_insert(document: dict, database: str, collection: str) -> dict:
    """Insert via pymongo (replaces flaky MCP subprocess)."""
    from pymongo import MongoClient
    client = MongoClient(MONGO_URI, serverSelectionTimeoutMS=5000)
    try:
        res = client[database][collection].insert_one(document)
        return {"tool_used": "pymongo.insert_one", "insertedId": str(res.inserted_id)}
    finally:
        client.close()


def _mcp_stdio_insert_legacy(document: dict, database: str, collection: str) -> dict:
    """Legacy MCP subprocess path — kept for reference."""
    proc = _spawn_mcp()
...
def run(run_id: str, data: dict) -> dict:
    ...
    push_log(run_id, "Model: meta-llama/llama-3.3-70b:free | MCP transport: stdio")
