...
if not app.secret_key:
    app.secret_key = "tache1-dev-secret-key"
    app.logger.warning("Using dev secret key for Tache 1.")
...
# -- Portal-managed auth verification --
@app.before_request
def _verify_portal_session():
    # In a professional setup, we would verify the session cookie or a JWT
    # shared between the gateway and this service.
    # For now, we ensure the user is at least present in the session.
    if "user" not in session and request.endpoint not in ["static", "login"]:
        # If not in session, we might want to return a 401 or redirect
        # depending on whether it's an API or UI request.
        pass


# -- Authentication is now handled by the SBT Portal gateway (port 5050).
#    The /login, /signup, /logout routes have been removed from tâche 1.
...
if __name__ == "__main__":
    # host=0.0.0.0 → reachable from phones on the same Wi-Fi (mobile support)
